...
const TestimonialsSection = () => {
    ...
    // slidesPerView updates with window size
    useEffect(() => {
        const updateSlides = () => {
    ...
    // make sure currentIndex stays within bounds when slidesPerView changes
    useEffect(() => {
        ...
        <section className="bg-background py-20 overflow-hidden">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="relative mb-16">
                    ...
                    {/* Navigation */}
                    <div className=" md:absolute  md:right-0 md:top-0 flex justify-center gap-4 mt-10 md:mt-0 ">
                        <Button
                            variant="ghost"
                            size="icon"
                            onClick={prevTestimonial}
                ...
                <div className="relative overflow-hidden">
                    <div
                        className="flex transition-transform duration-700 ease-in-out"
                        style={{
                            // Correct transform: move by pages in track-percentage units
                            transform: `translateX(-${(currentIndex * slidesPerView * 100) / testimonials.length}%)`,
                            // Track width = number of slides / slidesPerView * 100%
                    ...
                    >
                        {testimonials.map((testimonial, index) => (
                            <div
                                key={index}
                                className="w-1/3 flex-shrink-0 px-6"
                                // slide width relative to track (keeps layout identical)
                                style={{ width: `${100 / testimonials.length}%` }}
                            >
                                <div className="space-y-6 opacity-100 transform transition-all duration-500">
                                    <div className="flex justify-center">
